...
void CGxxListCtrl::DrawSubItem( CDC* pDC,int nItem,int nSubItem,UINT itemState )
{
	...
	if( !GetColIsShared(nSubItem) ) //添加此判断为了避免闪烁
	{
		...
		if( pWnd != NULL && IsWindow(pWnd->m_hWnd) && pWnd->IsWindowVisible())
		{
			CRect rcWn;
...
CWnd* CGxxListCtrl::CreateChildWnd( int nChildWndKind )
{
	...
	case GLC_Button:
		{
			pWnd = new CButton;
			((CButton *)pWnd)->Create(_T(""),BS_PUSHBUTTON|WS_VISIBLE|WS_CHILD|BS_NOTIFY,\
	...
	case GLC_CheckBox:
		{
			pWnd = new CButton;
			((CButton *)pWnd)->Create(_T(""),BS_AUTOCHECKBOX|WS_VISIBLE|WS_CHILD|BS_NOTIFY,\
				CRect(0,0,0,0),this,++m_nInnerMaxCtrlID );
		}
		break;
	case GLC_RadioBox:
		{
			pWnd = new CButton;
			((CButton *)pWnd)->Create(_T(""),BS_AUTORADIOBUTTON|WS_VISIBLE|WS_CHILD|BS_NOTIFY,\
				CRect(0,0,0,0),this,++m_nInnerMaxCtrlID );
		}
		break;
	case GLC_ComboBox:
	...
	case GLC_Static:
		{
			pWnd = new CStatic;
			((CStatic *)pWnd)->Create(_T(""),WS_VISIBLE|WS_CHILD, \
				CRect(0,0,0,0),this,++m_nInnerMaxCtrlID );
		}
		break;
	case GLC_Edit:
		{
			pWnd = new CEdit;
			((CEdit *)pWnd)->Create(WS_TABSTOP|WS_VISIBLE|WS_CHILD|ES_AUTOHSCROLL|ES_WANTRETURN,\
				CRect(0,0,0,0),this,++m_nInnerMaxCtrlID );
...
BOOL CGxxListCtrl::PreTranslateMessage(MSG* pMsg)
{
	...
	{
		UINT nID = (UINT)((LPNMHDR)pMsg->lParam)->idFrom;
		int iItem,iItem2;
...
BOOL  CGxxListCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	LPNMHDR lpHdr = (LPNMHDR)lParam;
	int iItem,iItem2;
	GetItemIdByChildWndID( (UINT)lpHdr->idFrom, &iItem, &iItem2);
	if (iItem!=-1 && iItem2!=-1)
	{
...
BOOL  CGxxListCtrl::OnCommand(WPARAM wParam, LPARAM lParam)
{
...
BOOL  CGxxListCtrl::GetColIsAutosize(int iCol)
{
	return m_vecColsInfo[iCol].bAutoAdjust;
}

UINT  CGxxListCtrl::RegeisterCtrlID()
{
	return ++m_nInnerMaxCtrlID;
}

BOOL  CGxxListCtrl::OnEraseBkgnd(CDC* pDC)
{
