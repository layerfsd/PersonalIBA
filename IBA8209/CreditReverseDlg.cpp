...
void CCreditReverseDlg::OnBnClickedOk()
{
	...
	if (CreditReverse.GetStatusCode() == 0)
	{
		...
		CLocalServer::GetInstance()->ActiveMemberList.UpdateActiveMember(
			m_nMemberId, 0, 0, CIBAHelpper::DecT2Int(m_strCreditMoney));
		 //发送充值消息		
		CString strTmp;
		strTmp.Format(IDS_ONLINECREDITREVERSE, CIBAHelpper::DecT2Int(m_strCreditMoney) / 100.0);
		...
		OperationInfo.SetOperationAmount(CIBAHelpper::DecT2Int(m_strCreditMoney));
	...
	*pResult = 0;
	...
	if ( lpGLC->nColID == eModify )
	{
		...
		if (dlgMod.DoModal() == IDOK)
		{
			...
			UINT nNewMoney = CIBAHelpper::DecT2Int(strNewMoney);
...
BOOL CCreditReverseDlg::DeleteCredit(BOOL bSendMsgToClient)
{
	...
	if (CreditReverse.GetStatusCode() == 0)
	{
		...
		CLocalServer::GetInstance()->ActiveMemberList.UpdateActiveMember(
			m_nMemberId, 0, 0, CIBAHelpper::DecT2Int(m_strCreditMoney)+ CreditReverse.GetCreditPresent());
		if (bSendMsgToClient)
		{
			//发送充值消息		
			CString strTmp;
