...
BOOL CServletHellper::ActivationMember(UINT nMemberID, UINT nActivationType)
{
	//中心是否开启激活设置
...
BOOL CDeregisterHelpper::DeregisterUser(CBalanceMethod & Balance)
{
	...
	{	
		...
		if(CIBAGlobal::emRealnameZD == CNetBarConfig::GetInstance()->GetAuditType())
		{
			...
			_tcsncpy(coi.szName, Balance.GetUserName(), 100);
			_tcsncpy(coi.szNetId, Balance.GetSerailNum(), 100);
			_tcsncpy(coi.szTermId, Balance.GetTermId(), 50);

			CString strIP;
			CIBADAL::GetInstance()->GetIPFromTermId(coi.szTermId, strIP);
			_tcsncpy(coi.szIP, strIP, 50);
			coi.nCheckoutType = Balance.GetIsMember() ? CIBAGlobal::cot_MemLogout : CIBAGlobal::cot_TempLogout;
