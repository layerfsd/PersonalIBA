...
BOOL CSeat::InitImage(LPCTSTR szSeatPath)
{
	...
	if(NULL == m_StatusBitmap)
	{
		...
		if(NULL == szSeatPath)
		{
			bmp = LoadBitmap(theApp.GetResHandle(), MAKEINTRESOURCE(IDB_STATUS));
		}
		else
		{
			bmp = (HBITMAP)::LoadImage(NULL, szSeatPath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
		}
...
BOOL CSeat::UpdateComputerStatus()
{
	...
	case CComputerInfo::ECS_ONLINE:
		...
		switch(info.GetUserClassId())
		{
		case Euc_PostPaid:			// 后付费
			nNewStatus = 10;
			break;

		case Euc_Employee:			// 员工卡
			nNewStatus = 13;
			break;
...
void CSeat::DrawTermID(CDC* pDC, int x, int y)
{
	...
	y += GetRadius() * 2 - 3;
	CRect rc(x, y, x + GetRadius() * 2, y + 20);
	CFont font;
