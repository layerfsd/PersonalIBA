...
{
	if(m_strExportExcel.IsEmpty() || m_strFullPath.IsEmpty())
	{
		return;
	}
	...
	{	
		...
		exEp.Format_PutNumberFormat(exEp.GetActiveSheet(), NS_GXX::CExcel::_E_, nBegRow, NS_GXX::CExcel::_E_, nAllRowCount,"@");
		exEp.Flush();
		exEp.Format_PutNumberFormat(exEp.GetActiveSheet(), NS_GXX::CExcel::_G_, nBegRow, NS_GXX::CExcel::_G_, nAllRowCount,"@");
		exEp.Flush();

		USES_CONVERSION;		// 使用W2A（）需要调用这个宏
		exEp.AppendData(W2A(m_strExportExcel));
		exEp.Flush();
