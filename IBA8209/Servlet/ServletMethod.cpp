...
void CServletMethod::OnEnd(BOOL bRet)
{
	...
	CIBALog::GetInstance()->WriteFormat(_T("CServletMethod::Status - %s"), strTmp);
...
BOOL CServletMethod::Execute()
{
	...
	CIBALog::GetInstance()->WriteFormat(_T("CServletMethod::Execute - %s"), GetMethodName());
	//重置
	m_RespFields.ResetAllValue();
	m_RespFields.SetValue(_T("Status"), _T("-1")); //重置为-1
	...
	bRet = GetCenterData(strParams, strResp);
...
BOOL CServletMethod::GetHttpData(LPCTSTR lpszURL, CString& strResp)
{
	...
	{
		CIBALog::GetInstance()->Write(_T("CServletMethod::GetHttpData Error!"));
	...
	if (HttpRequest.GetReadyState() == 4)//连接成功
	{
		...
		if (strResp.GetAt(0) == '<') //正在升级
		{
			...
			CIBALog::GetInstance()->Write(_T("Center is being upgraded!"));
