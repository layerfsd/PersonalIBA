...
void __gxx_trace__::_traceW(wchar_t * wc)
    {
        ::WideCharToMultiByte( _AtlGetConversionACP(), 0, wc, wcslen(wc)+1, _buf, 1024, NULL, NULL );
        _traceA(_buf);
    }
...
__gxx_auto_timer__::~__gxx_auto_timer__()
{
	...
	{
		int nlen = wcslen(_wspaces);
		wsprintf(&_wspaces[nlen],L":%d",_dwTick);
...
void __gxx_log__::operator ()(const wchar_t* tcszLogFilePath)
{
	...
	int nLen = wcslen(tcszLogFilePath);
...
void __gxx_log__::operator ()(int iWay,const char* szOutFormat,...)
{
    ...
    char _buf[1024];
    vsnprintf_s(_buf,1024,1024,szOutFormat,args);
    va_end(args);
    ...
    GetLocalTime(&stTime);
    char outbuf[1024];
	sprintf_s(outbuf,1024,"%s! [ %d-%d-%d %d:%02d:%02d  ]: %s\r\n",
                          cOut,
...
void __gxx_log__::operator () (int iWay,const wchar_t * tcszOutFormat,...)
{
    ...
    vswprintf_s(_wBuf,tcszOutFormat,args);
    va_end(args);
    ...
    GetLocalTime(&stTime);
    wchar_t outbuf[1024];
    swprintf_s(outbuf,1024,L"%s! [ %d-%d-%d %d:%02d:%02d  ]: %s\r\n",
        tOut,
        stTime.wYear,stTime.wMonth,stTime.wDay,
        stTime.wHour,stTime.wMinute,stTime.wSecond,
        _wBuf);
...
void __gxx_log__::LogCreate()
{
	...
	WCHAR chBuf[512];
	swprintf_s(chBuf,L"%s%04d%02d%02d.log",m_chFilePath,stToday.wYear,stToday.wMonth,stToday.wDay);
...
void __gxx_log__::log_error( const wchar_t* tcszOutFormat,... )
{
	...
	va_list  args;
	va_start(args,tcszOutFormat);
	vswprintf_s(_wBuf,2048,tcszOutFormat,args);
	va_end(args);
