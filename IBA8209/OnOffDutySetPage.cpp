...
void CSetPageOfDuty::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_NOTSHOW_DUTYBTN, m_bNotShowChgDutyBtn);
	...
	DDX_Control(pDX, IDC_EDIT_FIX_RESERVE, m_editFixReserve);
}


BEGIN_MESSAGE_MAP(CSetPageOfDuty, CIBAPage)
	ON_CBN_SELCHANGE(IDC_COMBO_SHIFTCOUNT, OnCbnSelchangeComboShiftcount)
	ON_BN_CLICKED(IDC_CHECK_DYNAMICREVERSE, OnBnClickedCheckDynamicreverse)
END_MESSAGE_MAP()
...
void CSetPageOfDuty::OnOK()
{
	...
	/////////////////不显示交班按钮//////////////////////////
	CIBAConfig::GetInstance()->SetNotShowChgDutyBtn(m_bNotShowChgDutyBtn);
	/////////////////上交金额///////////////////
	...
	//固定备用金
	m_editFixReserve.GetWindowText(strTmp);
	double dblReverveAmount = _tstof(strTmp);
	strTmp.Format(_T("%0.2f"), dblReverveAmount);	// 保留两位就可以
	CIBAConfig::GetInstance()->SetFixReserve(strTmp);
	CIBAPage::OnOK();
}
...
BOOL CSetPageOfDuty::OnInitDialog()
{
	...
	// 固定备用金
	m_editFixReserve.EnableWindow(!m_bDynamicReverseMoney);

	// 不显示交班按钮
	m_bNotShowChgDutyBtn = CIBAConfig::GetInstance()->GetNotShowChgDutyBtn();
	//上交金额
	...
	m_editFixReserve.SetValidChars(_T("0123456789."));
	m_editFixReserve.SetLimitText(7);
	CString strFixReserve = CIBAConfig::GetInstance()->GetFixReserve();
	m_editFixReserve.SetWindowText(strFixReserve);
...
void CSetPageOfDuty::OnBnClickedCheckDynamicreverse()
{
	if(((CButton*)GetDlgItem(IDC_CHECK_DYNAMICREVERSE))->GetCheck())
	{
		m_editFixReserve.EnableWindow(FALSE);
	}
	else
	{
		m_editFixReserve.EnableWindow(TRUE);
	}
}
