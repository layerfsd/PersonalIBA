...
void CSeatLayoutView::LoadSeatBitmap(CGdiPlusBitmap * pSeatBitmap)
{
	...
	if(strPath.IsEmpty())
	{
		HBITMAP hBitmap = (HBITMAP)::LoadBitmap(theApp.GetResHandle(), MAKEINTRESOURCE(IDB_BG));
		if (hBitmap != NULL)
		{
			m_pSeatBitmap->m_pBitmap = Gdiplus::Bitmap::FromHBITMAP(hBitmap, NULL);
			m_pSeatBitmap->SetLoadSuccess();
	...
	case M_SEAT_COMPUTER_MENU:
		{
			...
			{//进入元素区域，即移动操作
				...
				strTmp.LoadString(ID_SEAT_TERMINALINFO);		// 删除剩余上网时长
