...
void CActiveMemberView::OnFilePrint()
{
	CActiveMember ActiveMember;
	...
	if (GetCurActiveMember(ActiveMember))
	{
		...
		CIBAPrintA4::GetInstance()->m_strName = ActiveMember.GetUserName();
		CIBAPrintA4::GetInstance()->m_strNumber = ActiveMember.GetNetId();
		CIBAPrintA4::GetInstance()->m_strID = ActiveMember.GetPersonalID();
		CIBAPrintA4::GetInstance()->m_strAddress = _T("");

		CChinaUnicomConfig::ClassIDItem item;
		if (CChinaUnicomConfig::GetInstance()->GetClassIDItem(ActiveMember.GetClassId(), item))
		{
...
void CActiveMemberView::OnUpdatePrint(CCmdUI *pCmdUI)
{
	pCmdUI->Enable(FALSE);
