...
BOOL CWDRealName::ScanUserInfo(UINT nType, CNetBarUser& NetBarUser, UINT nOption)
{
	...
	case 1:
		...
		if (m_pfnGetInfoFirstType != NULL)
		{
			nRet = m_pfnGetInfoFirstType(&wdUserInfo);
		}
		else
		{
	...
	case 2:
		...
		if (m_pfnGetInfoSecondType != NULL)
		{
			nRet = m_pfnGetInfoSecondType(&wdUserInfo);
		}
		else
		{
	...
	case 3:
		...
		if (m_pfnGetInfoSecondTypeMachine != NULL)
		{
			nRet = m_pfnGetInfoSecondTypeMachine(&wdUserInfo);
		}
		else
		{
	...
	default:
		...
		if (m_pfnGetOtherType != NULL)
		{
			nRet = m_pfnGetOtherType();
		}
		else
		{
	...
	if (nRet)
	{
		try
		{
			NetBarUser.SetCertificateID(CString(CA2T(wdUserInfo.szID)));
			NetBarUser.SetUserName(CString(CA2T(wdUserInfo.szName)));
			NetBarUser.SetAddr(CString(CA2T(wdUserInfo.szAddress)));
			NetBarUser.SetNation(CString(CA2T(wdUserInfo.szNation)));
			NetBarUser.SetDepartment(CString(CA2T(wdUserInfo.szFromDepart)));
			...
			if (CString(CA2T(wdUserInfo.szSex)).CompareNoCase(_T("男")) != 0)
			{
