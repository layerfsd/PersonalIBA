...
//调试参数初始化，以免需要到后台设置
	...
	{
		//QSCTT AfxMessageBox(IDS_CONNETCTDBFIALED);
...
void CNetBarConfig::LoadFromIni()
{
	...
	ReadValue(_T("Center"), _T("MainPath"), m_MainCenterPath, _T("netbar"));

	ReadValue(_T("Center"), _T("BackUpIP"), m_BackupCenterIP);
	ReadValue(_T("Center"), _T("BackUpPort"), m_BackupCenterPort);
	ReadValue(_T("Center"), _T("BackUpPath"), m_BackupCenterPath, _T("netbar"));
...
BOOL CNetBarConfig::PraseControlOption(CString strInfo)
{
	...
	for (INT i = 0; i < m_strControlOptionArray.GetCount(); i++)
	{
		...
		if (GetValue(strTmp, _T("gintsoftsfreg"), m_nSoftSfreg))
		{
			continue;
		}
