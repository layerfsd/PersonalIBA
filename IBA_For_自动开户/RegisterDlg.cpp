...
void CRegisterDlg::OnBnClickedOk()
{
	...
	// 2013-3-13 15:25:51 qsc
	//0:无实名 1:任子行 2:过滤王 3:摩根 4:桑地 5:网盾 6.任子行2008
	//if(5 == CNetBarConfig::GetInstance()->GetAuditType())
	//{
	//	// 实名如果是网盾的时候就需要判断一下的SunlikeAuditProxy的配置文件里面是否有一个配置的是网盾实名中心
	//	// 一般情况下SunlikeAuditProxy和IBA放在同一个目录下
	//	CString strPath;
	//	GetModuleFileName(NULL, strPath.GetBuffer(MAX_PATH), MAX_PATH);
	//	strPath.ReleaseBuffer();
	//	strPath = strPath.Left(strPath.ReverseFind('\\') + 1);		// 去除IBA.exe

	//	strPath += _T("AUDITPROXY.INI");

	//	INT nCenterCount = GetPrivateProfileInt(_T("CENTERCOUNT"), _T("COUNT"), 0, strPath);
	//	BOOL bWDExists = FALSE;
	//	// 实名中心初始化
	//	for (int i= 0; i<nCenterCount; i++) 
	//	{
	//		CString strSection;
	//		strSection.Format (_T("CENTERUPLOAD_%d"), i);
	//		INT nCenterType = GetPrivateProfileInt(strSection, _T("CENTERTYPE"), -1, strPath) ;
	//		// 1为网盾实名中心	
	//		if(1 == nCenterType)
	//		{
	//			bWDExists = TRUE;
	...
	//	if(!bWDExists)
	//	{
	//		theApp.IBAMsgBox(_T("实名异常，请修改SunlikeAuditProxy的配置!"));
	//		return;
	//	}
	//	
	//}
...
BOOL CRegisterDlg::OnInitDialog()
{
	...
	IBA_LOG0(_T("Register dialog init start!"));
...
void CRegisterDlg::ExeSfreg()
{
	...
	strParam.Format(_T("-A%s -B%s -C%s -D%s -E%s -F%s -G%s -H%s -I%d -J%s -K%s -L%s -M%s -R%d"), 
		m_strPersonalID, strIDType, m_strUserName, strSex,
		m_userInfo.GetAddr(), m_userInfo.GetTel(), _T("CN"), m_strNetId, 
		m_nIdType, m_strNation, m_strOfficer, m_bIsMember,
		_T(""), m_bCanEditUserInfo);
