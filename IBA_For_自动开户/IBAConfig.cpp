...
, m_bShowLastUserNetId(FALSE), m_bNotShowChgDutyBtn(FALSE), m_bCanNotTermConsumeQuery(FALSE)
{
	m_strIniFile = theApp.GetWorkPath();
	m_strIniFile.Append(_T("\\IBAConfig\\IBA2.ini"));
	...
	GPropertyInit(IsShowShiftCash, TRUE);
	GPropertyInit(CameraIndex, 0);
...
UINT CIBAConfig::GetLocalSerialNo()
{
	...
	m_LocalSerialNo = AfxGetApp()->GetProfileInt(_T("NetBarInfo"), _T("LocalSerialNo2"), 0);
	...
	AfxGetApp()->WriteProfileInt(_T("NetBarInfo"), _T("LocalSerialNo2"), m_LocalSerialNo);
...
void CIBAConfig::LoadConfig()
{
	...
	// 不允许终端消费查询
	ReadValue(_T("IBA"), _T("CanNotTermConsumeQuery"), m_bCanNotTermConsumeQuery, FALSE);
	...
	// 不显示交接班按钮
	ReadValue(_T("Duty"), _T("NotShowChgDutyBtn"), m_bNotShowChgDutyBtn, 0);
	...
	ReadValue(_T("IBA"), _T("IsShowShiftCash"), SET(IsShowShiftCash), TRUE);

	// 2013/09/12-qsc: 
	ReadValue(_T("IBA"), _T("CameraIndex"), SET(CameraIndex), 0);

	ReadValue(_T("Set2"), _T("LastOperator2"), m_strLastOperator, _T(""));
}
...
void CIBAConfig::SaveConfig()
{
	...
	SaveValue(_T("Duty"), _T("NotShowChgDutyBtn"), m_bNotShowChgDutyBtn);
	...
	SaveValue(_T("IBA"), _T("IsShowShiftCash"), GET(IsShowShiftCash));

	SaveValue(_T("IBA"), _T("CameraIndex"), GET(CameraIndex));	

	ReadValue(_T("Set2"), _T("LastOperator2"), m_strLastOperator);
}
