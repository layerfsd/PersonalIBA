...
class CJXCExchangeDlg : public CNDDialog
{
	DECLARE_DYNAMIC(CJXCExchangeDlg)
	DECLARE_MESSAGE_MAP()

public:
	CJXCExchangeDlg(CWnd* pParent = NULL);   // 标准构造函数
	virtual ~CJXCExchangeDlg();

	void AddSellRecord(NS_SERVLET::CSellRecord& SellRecord);

// 对话框数据
	enum { IDD = IDD_JXC_EXCHANGE };
