...
BEGIN_MESSAGE_MAP(CMainFrame, CBCGPFrameWnd)
	...
	ON_COMMAND(IDM_AUTO_REGISTER, OnAutoRegister)
...
void CMainFrame::OnCreateEnd()
{
	SetTimer(1007, 10 * 1000, NULL);	// 十秒后保存取filelist的地址
...
{
	if (!theApp.GetRealName()->DoScan(nId - IDM_SCAN1 + 1))
	{
...
void CMainFrame::OnUpdateDianka(CCmdUI *pCmdUI)
{
	...
	pCmdUI->Enable(theApp.GetCurCashier()->IsOnDuty() && theApp.GetCurCashier()->HasPopedomOf(_T("IBAP_POINTCARD")));
}
...
void CMainFrame::OnAutoRegister()
{
	if (!IsWindow(m_dlgAutoRegister.m_hWnd))
	{
		m_dlgAutoRegister.Create(IDD_AUTO_REGISTER, this);
	}
	m_dlgAutoRegister.ShowWindow(SW_SHOW);
}
