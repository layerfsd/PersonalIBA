...
void CCashier::WriteLoginInfo(CString strOperator)//登录后写注册表
{
	...
	theApp.WriteProfileString(_T("NetBarInfo"), _T("LastOperator2"), strOperator);

	//查找注册表
	CString strTmp = theApp.GetProfileString(_T("NetBarInfo"), _T("Operators2"));
	strTmp.Trim();
	if (strTmp.IsEmpty())
	{
		//添加所有用户到注册表
		...
		theApp.WriteProfileString(_T("NetBarInfo"), _T("Operators2"), strTmp);
	}
}
...
BOOL CCashier::DoConsumeListEx(CConsumeListMethodEx & ConsumeListEx)
{
	...
	::CoInitialize(NULL);
	BOOL bRet = ConsumeListEx.Execute();
	::CoUninitialize();
	return bRet;
}
