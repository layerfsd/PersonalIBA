	...
	if (pRow != NULL) 
	{
		...
		if ( (int(m_nMemberId)) < 0 ) 
		{
			...
			else
			{
				...
				if(IDOK == dlg.DoModal())
				{
					strTmp = CIBAHelpper::EncodePassword(dlg.GetPwd(), false);
			...
			IBA_LOG(_T("Temp.CNDCheckInDlg::OnBnClickedOk Line 259 AddLastUserInfo"));
		}
		else
		{
			// 2014-10-27 - qsc 智多实名，手动输入密码
			...
			if (!GetCheckIn() && !ActiveMember.GetTerminalID().IsEmpty())
			{
				...
				{
					// 2014-10-30 - qsc 智多实名下使用对话框输入的密码
					if(CIBAGlobal::emRealnameZD != CNetBarConfig::GetInstance()->GetAuditType())
					{
				...
				CIBADAL::GetInstance()->DeleteLastUserInfo(LastUserInfo.GetTermId());
				CIBADAL::GetInstance()->AddLastUserInfo(LastUserInfo);
				IBA_LOG(_T("Temp.CNDCheckInDlg::OnBnClickedOk Line 304 AddLastUserInfo"));
			...
			else
			{
				...
				CIBADAL::GetInstance()->DeleteLastUserInfo(LastUserInfo.GetTermId());
				CIBADAL::GetInstance()->AddLastUserInfo(LastUserInfo);
				IBA_LOG(_T("Temp.CNDCheckInDlg::OnBnClickedOk Line 325 AddLastUserInfo"));
			}
		}
...
/*************************************************
Function:       // CheckTakeUp
Description:    // 使用终端号检查终端是否被占用，从LastUserInfo表中查询，
				// Suspend字段=1，被占用
				// UpdateTime距离现在不超过10分钟，被占用
Calls:          // GetLastUserInfo,
Table Accessed: // LastUserInfo
Table Updated:  //
Input:          // strTermId - 待查终端号
Output:         // None
...
BOOL CNDCheckInDlg::CheckTakeUp(CString strTermId)
{
	...
	if (CIBADAL::GetInstance()->GetLastUserInfo(LastUserInfo, 0, strTermId))
	{
		...
		{
			IBA_LOG(_T("Temp.CNDCheckInDlg::CheckTakeUp: Suspend =1"));
			bTakeUp = TRUE;
		}
		else//非挂机
		{
			...
			IBA_LOG(_T("Temp.CNDCheckInDlg::CheckTakeUp: Suspend =1 , updateTime = %s, comsumetime = %d 分钟"),updateTime.Format(_T("%Y-%m-%d %H:%M")),interval.GetTotalMinutes());	

			if (interval.GetTotalMinutes() < 10)
			{
				IBA_LOG(_T("Temp.CNDCheckInDlg::CheckTakeUp: interval of minutes <10 : %d"),interval.GetTotalMinutes());
				bTakeUp = TRUE;
			} 
		}
	}
...
BOOL CNDCheckInDlg::DoLocalModeQuery()
{
	...
	if (CIBAGlobal::NetworkStatus == CIBAGlobal::emNetworkBroken &&
			m_ActiveMember.GetIsLocalUser() == FALSE &&
			GetCheckIn() == TRUE)
	{
		MsgBox(_T("中心用户不允许在本地模式下自动上机"));
		CBCGPDialog::OnOK();
		return TRUE;
	}

	m_strPwd = m_ActiveMember.GetPassword();
