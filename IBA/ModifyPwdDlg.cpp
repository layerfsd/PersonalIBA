...
void CModifyPwdDlg::OnBnClickedOk()
{
	...
	{
		if (m_ActiveMember.GetIsLocalUser() == FALSE)
		{
	...
	else if (m_ActiveMember.GetIsLocalUser())
	{
		DoLocalModeOperate();
	...
	{
		//密码长度不得>8位
		if(8 < m_strPwd.GetLength())
		{
			SetToolTipPos(IDC_EDIT_PASSWORD);
			ShowToolTip(_T("密码长度不能大于8位!"));
			return;
		}
...
BOOL CModifyPwdDlg::OnInitDialog()
{
	...
	//((CEdit*)GetDlgItem(IDC_EDIT_PASSWORD))->SetLimitText(8);//为了在超出长度时显示提示
...
BOOL CModifyPwdDlg::DoLocalModeOperate()
{
	...
	m_ActiveMember.SetPassword(m_strPwd);

	BOOL bRet = CLocalServer::GetInstance()->ActiveMemberList.UpdateActiveMemberDB(m_ActiveMember);
