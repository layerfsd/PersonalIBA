...
BOOL CHoverButton::LoadBitmap(HINSTANCE hRes, UINT bitmapid, UINT bmpCount)
{
	DeleteObject(m_Bitmap.Detach());

	HGDIOBJ hImage = LoadImage(hRes, MAKEINTRESOURCE(bitmapid), IMAGE_BITMAP, 0, 0,LR_LOADMAP3DCOLORS);
...
BOOL CHoverButton::LoadPng(HINSTANCE hRes, UINT pngid, UINT bmpCount /*= 3*/)
{
	DeleteObject(m_Bitmap.Detach());

	CImageEx MyImage;

	if (MyImage.LoadFromOtherResource(hRes, pngid, _T("PNG")))
	{
