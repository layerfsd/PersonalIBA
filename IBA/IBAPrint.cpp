...
void CIBAPrint::Print(CString& strContent)
{
	...
	//2015-0821 liyajun 改字体为细体
	//lf.lfWeight = FW_BOLD;
	lf.lfWeight = FW_NORMAL;
	lf.lfOutPrecision = OUT_TT_ONLY_PRECIS;
	lstrcpy(lf.lfFaceName, _T("黑体"));
	//lstrcpy(lf.lfFaceName, _T("仿宋—GB2312"));
...
void CIBAPrint::GetShoppingTicket(CString& strContent, BOOL bIsCredit)
{	
	CString strFile(theApp.GetWorkPath());

	if (bIsCredit)
		strFile.Append(_T("\\IBAConfig\\PrintTemplate\\CreditShopping.txt"));
	else
		strFile.Append(_T("\\IBAConfig\\PrintTemplate\\AccountShopping.txt"));

	strContent.Empty();

	CIBAHelpper::ReadUtf8File(strFile, strContent);
