...
{	
	...
	else
	{
		...
		if (CIBAConfig::GetInstance()->GetDynamicReserve())
 		...
 		else
 		{
			...
			// 2014-2-28 - qsc
			m_edtReverveAmount.SetWindowText(CIBAConfig::GetInstance()->GetFixReserve());
	...
	else
	{
		...
		{
			GetDlgItem(IDC_STATIC_WXTEXT)->ShowWindow(SW_SHOW);
			GetDlgItem(IDC_STATIC_WXAMOUNT)->ShowWindow(SW_SHOW);
		}
	}

	// 不显示下班按钮
	if(CIBAConfig::GetInstance()->GetNotShowChgDutyBtn())
	{
...
void CDlgIBADuty::PrintTicket(CShiftCash &ShiftCash)
{
	...
	if(CNetBarConfig::GetInstance()->GetOpenWx())
	{
		// 2014-11-5 - qsc 在充值卡金额后面插入数据
		CString strFind(_T("[creditcard]"));
		int nFind = strTicket.Find(strFind);
		CString strL = strTicket.Left(nFind + strFind.GetLength());
		CString strR = strTicket.Right(strTicket.GetLength() - nFind - strFind.GetLength());
		strTicket = strL + _T("\r\n微信充值金额：[wxpayamount]") + strR;

		strTmp.Format(_T("%.2f"),ShiftCash.GetWxPayAmount() / 100.0);
		strTicket.Replace(_T("[wxpayamount]"), strTmp);
	}
...
void CDlgIBADuty::OnBnClickedButtonPrint()
{
	...
	if (CIBAConfig::GetInstance()->GetShiftReportType() == 0)
	{
		...
		int nVer = 2;
		if(PathFileExists(strReportDllPath))
		{
			CFileVersionInfo fvi;
			if (fvi.Create(strReportDllPath))
			{				CString  strVer = fvi.GetFileVersion();				strVer.Trim();
				IBA_LOG(_T("DdnReport.dll %s"), strVer);
				// 新版本DdnReport 2开头
				nVer = _ttoi(strVer.Left(1));
			}
			else
			{
				IBA_LOG0(_T("获取DdnReport.dll版本失败"));
			}
		}
		else
		{
		...
		if(nVer >= 2)
		{
			CIBAReportHelper::GetInstance()->m_strNetbarName = CNetBarConfig::GetInstance()->GetNetBarName();

			CIBAReportHelper::GetInstance()->m_strValue1 = theApp.GetCurCashier()->GetName();
			CIBAReportHelper::GetInstance()->m_strValue2 = m_strOnDutyTime;
			CIBAReportHelper::GetInstance()->m_strValue16 = CIBAHelpper::GetCurrentTimeAsString(TRUE);
			CIBAReportHelper::GetInstance()->m_strValue15 = CIBAHelpper::GetCurrentTimeAsString(TRUE);
			CIBAReportHelper::GetInstance()->m_strValue3.Format(L"%.2f", m_ShiftTotalMethod.GetCreditAmount() / 100.0);
			CIBAReportHelper::GetInstance()->m_strValue9.Format(L"%.2f", m_ShiftTotalMethod.GetReturnedAmount() / 100.0);
			CIBAReportHelper::GetInstance()->m_strValue4.Format(L"%.2f", m_ShiftTotalMethod.GetCashPay() / 100.0);
			CIBAReportHelper::GetInstance()->m_strValue10.Format(L"%.2f",  m_ShiftTotalMethod.GetDiankaAmount() / 100.0);
			CIBAReportHelper::GetInstance()->m_strValue5.Format(L"%.2f", m_ShiftTotalMethod.GetStoreInAmount() / 100.0);
			CIBAReportHelper::GetInstance()->m_strValue11.Format(L"%.2f", nPayout / 100.0);	// 支出

			CIBAReportHelper::GetInstance()->m_strValue17.Format(L"%.2f", (m_nDutyIncome - nPayout) / 100.0);			// 班次收入
			CIBAReportHelper::GetInstance()->m_strValue12.Format(L"%.2f", m_ShiftTotalMethod.GetReserveAmount() / 100.0);	// 上一班预留
			CIBAReportHelper::GetInstance()->m_strValue13.Format(L"%.2f", nRemainAmount / 100.0);	// 留给下一班
			CIBAReportHelper::GetInstance()->m_strValue18.Format(L"%.2f", (nFetchAmount + nRemainAmount) / 100.0);		// 现金总额

			CIBAReportHelper::GetInstance()->m_strValue19.Format(L"%.2f", nFetchAmount / 100.0);	// 上缴金额
			CIBAReportHelper::GetInstance()->m_strValue6.Format(L"%.2f", m_ShiftTotalMethod.GetCreditCard() / 100.0);
			CIBAReportHelper::GetInstance()->m_strValue14.Format(L"%.2f", m_ShiftTotalMethod.GetNetBarPay() / 100.0);
			CIBAReportHelper::GetInstance()->m_strValue20.Format(L"%.2f", m_ShiftTotalMethod.GetZSBConsume() / 100.0);
			CIBAReportHelper::GetInstance()->m_strValue8 = m_strDescription;
			CIBAReportHelper::GetInstance()->m_strValue7.Format(L"%.2f", m_ShiftTotalMethod.GetWxPayAmount() / 100.0);	// 微信充值金额
			CIBAReportHelper::GetInstance()->DoReport();
		}
		else
		{
	...
	else
	{
		...
		ShiftCash.SetWxPayAmount(m_ShiftTotalMethod.GetWxPayAmount());
...
void CDlgIBADuty::ShowShiftTotalInfo()
{
	...
	if(CNetBarConfig::GetInstance()->GetOpenWx())
	{
		strTmp.Format(_T("%.2f"), m_ShiftTotalMethod.GetWxPayAmount() / 100.0);
		GetDlgItem(IDC_STATIC_WXAMOUNT)->SetWindowText(strTmp);
	}
	...
	//CString str = theApp.GetCurCashier()->GetOnDutyTime();
	//strTmp += str;
