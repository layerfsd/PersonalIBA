...
public :
	...
	// 2015-3-12 17:48:38-gxx:
	void AddActiveMember(CActiveMember& ActiveMember);
	...
	//激活时添加用户，不调用RemoveActiveMember，反调用RemoveActiveMemberPostActivation
	void AddActiveMemberPostActivation(CBalanceMethod& Balance, 
		BOOL bRegisterBT=FALSE, // 是否开户包时
		UINT nPayout=0,
		int nBundleTimeType = CActiveMember::EPayType_Capped,
		int nScanType=0); // 开户包时费用
	...
	void UpdateActiveMember(UINT nMemberId, UINT nNewBlance);
...
public:
	...
	BOOL RemoveActiveMemberDBPostActivation(UINT nMemberId);
...
private :
	...
	BOOL ProcessActiveMemberInAbnormalCases(CBalanceMethod& balance,CActiveMember &ActiveMember);
