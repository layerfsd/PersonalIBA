...
Description:    // 从ActiveMemberMap中查找当前选中行的用户对象.
Calls:          // GetActiveMember(nMemberId, ActiveMember)
Output:         // 找到则保存在ActiveMember中
Return:         // 找到返回TRUE，否则返回FALSE
Others:         // 查找是通过当前选中行的MemberId. MemberId是何时被保存到Data中的？
*************************************************/
...
void CActiveMemberView::OnCredit()
{
	...
	if (GetCurActiveMember(ActiveMember))
	{
		...
		else 
		{
			...
			dlg.SetNetId(ActiveMember.GetNetId());
			//2015-0907 liyajun 重复UpdateActiveMember了，删除
			//if (dlg.DoModal() == IDOK) {
...
/*************************************************
Function:       // OnPayout
Description:    // 用户视图点击"结账"。检查当前激活用户信息。
				// 包房用户 - Popup NetbarRoomDlg
				// 网络正常 - Popup NDReturnDlg
				// 本地模式 - Set localActiveMember, Popup NDReturnDlg 
Calls:          // DoModal
Table Accessed: // None
Table Updated:  // None
Input:          // None
...
void CActiveMemberView::OnPayout()
{
	...
	if (GetCurActiveMember(ActiveMember)) //获取当前激活用户信息
	{
		// 2011/10/31-8210-gxx: 
		if (ActiveMember.GetMemberID() == ActiveMember.GetRoomMemberId()) //包房用户弹出包房界面
		{
			// 包房主账号，自动弹出包房管理界面
			CNetbarRoomDlg dlg;
			dlg.SetNetId(ActiveMember.GetNetId(),TRUE);
		...
		{
			if (CIBAGlobal::NetworkStatus == CIBAGlobal::emNetworkNormal) //网络正常弹出退款界面
			{
				CNDReturnDlg dlg;
				dlg.SetNetId(ActiveMember.GetNetId());
				dlg.DoModal();
			}
			else                                                          //本地模式设置本地用户弹出退款界面
			{
				CNDReturnDlg dlg;
				//dlg.SetLocalActiveMember(ActiveMember);
				dlg.SetNetId(ActiveMember.GetNetId());
...
/*************************************************
Function:       // FindUserAt
Description:    // 从用户视图列表中查找对应strKeyword(上网账号)的用户，返回其索引.
Calls:          // None
Table Accessed: // None
Table Updated:  // None
Input:          // nKeyType - 0 -表示匹配ID的最后几个字符，1 - 表示必须全字符匹配
                // strKeyword - 待匹配的账号
Output:         // None
Return:         // 匹配的用户的索引号
Others:         // None
...
int CActiveMemberView::FindUserAt(UINT nKeyType,CString strKeyword)
{
	...
	case 0:  // 匹配ID的最后几个字符
		{
			...
			//遍历用户视图中的用户，寻找其termId的最后几位是否与strKeyword一致，一致则返回其在列表中的索引号
	...
	default:
		{
			IBA_LOG(_T("Abnormal.int CActiveMemberView::FindUserAt:未指定的查找类型"));
...
/*************************************************
Function:       // SetSelUser
Description:    // 设置用户视图的当前选中行，到Map中查找对应对象，找到则在任务栏显示该用户信息，并设置CCurrentNetBarUser的NetId
Calls:          // SetCurSel, ShowUserInfo, CCurrentNetBarUser::GetInstance()->SetNetId
Table Accessed: // None
Table Updated:  // None
Input:          // nIndex - 用户视图列表的索引号
                // bSetFocus - 是否将焦点设置到Grid控件上
Output:         // None
Return:         // None
Others:         // None
*************************************************/
...
void CActiveMemberView::SetSelUser(int nIndex, BOOL bSetFocus)
{
	...
	if (GetCurActiveMember(ActiveMember))
	{
		((CMainFrame*)AfxGetMainWnd())->m_wndTaskPane.ShowUserInfo(ActiveMember);  //任务栏显示用户信息
		CCurrentNetBarUser::GetInstance()->SetNetId(ActiveMember.SetNetId());      //设置CCurrentNetBarUser的NetId
	}
