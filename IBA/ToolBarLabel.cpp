...
/*************************************************
Function:       // ThreadRun
Description:    // 定时向中心获取增收宝消费记录(ThirdPartPayList)，写成文本，通过消息通知DlgIBAFastlane。
Calls:          // Cashier::DoConsumeListEx , CActiveMemberList::GetActiveMember
*************************************************/
void CIBAFastLane::ThreadRun()
{
	IBA_LOG(_T("Start thread:Get ZSB info. from center"));
	while(TRUE)
	{
		{
		CString strs;
...
//#define MY_DEBUG
		...
		for(int i = 0; i < (int)clMethod.GetRecordCount() ; i++)
		{
			...
			CActiveMember AM;
			CLocalServer::GetInstance()->ActiveMemberList.GetActiveMember(nMemberID,AM);
...
void CIBAFastLane::StartThread()
{
	...
	if (m_hWnd != NULL)
	{
		...
		m_Thread.Start();//20160116-liyajun
	}	
}
