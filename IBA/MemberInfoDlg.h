...
public:
	...
	UINT GetItemCount() {return m_nItemCount;}

private:
	
	UINT m_nQueryStyle; // 查询方式

	CBCGPListCtrl m_MemberInfoList; //控件变量
...
private:
	...
	// 从本地查找数据
	void FindInfoFromLocal(BOOL bValidUser, BOOL bFullMatch = FALSE);

	// 从已激活用户列表查找数据
	void FindInfoFromActiveMemberList();
};
