...
{
	// 计算最右和最下坐标，以当前终端的坐标，加上三个半径计算（三个半径可以改变，根据实际效果来）
	CSeatLayoutView::SetMaxRightCmp(newSeat->GetPosition().x + newSeat->GetRadius() * 3);
	CSeatLayoutView::SetMaxBottomCmp(newSeat->GetPosition().y + newSeat->GetRadius() * 3);
...
BOOL CSeatList::ReadList()
{
	...
	do 
	{
		...
		{
			INT_PTR nIndex = CLocalServer::GetInstance()->ComputerList.Find(strIDTmp, strIPTmp);
			if(nIndex >= 0)	// 如果机器视图存在该机器才能添加
			{
