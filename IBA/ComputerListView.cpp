...
void CComputerListView::OnCreditOnline()
{
	...
	if (GetCurComputerInfo(ComputerInfo))
	{
		...
		if (!strTmp.IsEmpty() && (ComputerInfo.HasUser() 
			|| ComputerInfo.GetComputerStatus() == CComputerInfo::ECS_ONLINE))
		{	
			if (CIBAGlobal::NetworkStatus == CIBAGlobal::emNetworkNormal) {
				...
				if (CLocalServer::GetInstance()->ActiveMemberList.GetActiveMember(ComputerInfo.GetMemberID(), ActiveMember))
				{
					...
					dlg.SetNetId(strTmp);
					//2015-0915 liyajun 本地充值中有一次UpdateActiveMember，此处的UpdateActiveMember应去掉
					dlg.DoModal();
					//if (dlg.DoModal() == IDOK) 
					//{
...
{//取监控的统计数据
	...
	//2015-0824 liyajun 增加显示区域剩余机台信息
	CString strTemp;
	for(INT_PTR i =0 ;i < StatInfo.m_PCClassInfoArray.GetCount(); ++i)
	{
		strTemp.Format(LOAD_STRING(IDS_STATTEXT1),StatInfo.m_PCClassInfoArray.GetAt(i).strPCClassName,StatInfo.m_PCClassInfoArray.GetAt(i).nAvailableDevices);
...
/*************************************************
Function:       // OnPayout
Description:    // 机器视图点击"结账"。检查终端信息以及终端上对应的激活用户信息存在。
				// 网络正常 - Popup NDReturn窗口
				// 本地模式 - set localActiveMember，Popup NDReturn窗口
Calls:          // DoModal
Table Accessed: // None
Table Updated:  // None
Input:          // None
...
void CComputerListView::OnPayout()
{
	...
	if (!GetCurComputerInfo(ComputerInfo)) //获取当前终端信息
	{
		return;
	}
	...
	if (!CLocalServer::GetInstance()->ActiveMemberList.GetActiveMember(ComputerInfo.GetMemberID(), ActiveMember)) //根据MemberId获取机器上用户信息
	{
		return;
	}

	if (CIBAGlobal::NetworkStatus == CIBAGlobal::emNetworkNormal) //正常下，弹出退款窗口
	{
		CNDReturnDlg dlg;
		dlg.SetNetId(ActiveMember.GetNetId());
		dlg.DoModal();
	}
	else                                                         //本地模式下，设置localActiveMember弹出退款窗口
	{
		// 断网状态下结账
		CNDReturnDlg dlg;
		//dlg.SetLocalActiveMember(ActiveMember);
...
void CComputerListView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (GetCurComputerInfo(ComputerInfo))
	{
		...
		//2016-0201 liyajun
		if(FALSE == ComputerInfo.GetNetId().IsEmpty())
		{
			CCurrentNetBarUser::GetInstance()->SetNetId(ComputerInfo.GetNetId());
		}
...
{
	//DamagingTest code 2016-0310 liyajun
	//int *p = NULL;
	//*p = 0;
