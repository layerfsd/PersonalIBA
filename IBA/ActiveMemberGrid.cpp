...
void CActiveMemberGrid::UpdateRowData(CBCGPGridRow* pRow, CActiveMember& ActiveMember)
{
	...
	//上网账号|级别|余额|开始时间|
	pRow->GetItem(0)->SetValue(ActiveMember.GetNetId().GetString(), FALSE);  //账号
	if (ActiveMember.GetIsLocalUser())                                       //用户类型
	{
		CIBAString str;
	...
	pRow->GetItem(2)->SetValue(ActiveMember.GetAvailavleBalanceAsString().GetString(), FALSE); //余额
	pRow->GetItem(3)->SetValue(ActiveMember.GetActivationTimeAsString().GetString(), FALSE);   //激活时间

	//电脑|区域|上机时间|用时|
	pRow->GetItem(4)->SetValue(ActiveMember.GetTermIdInfo().GetString(), FALSE);        //终端号
	pRow->GetItem(5)->SetValue(ActiveMember.GetPCClass().GetString(), FALSE);           //区域
	pRow->GetItem(6)->SetValue(ActiveMember.GetCheckInTimeAsString().GetString(), FALSE); //上机时间
	pRow->GetItem(7)->SetValue(ActiveMember.GetUseTimeAsString().GetString(), FALSE);     //用时

	//费用|消费模式|预付款|明细|
	pRow->GetItem(8)->SetValue(ActiveMember.GetAmountAsString().GetString(), FALSE);      //费用
	pRow->GetItem(9)->SetValue(ActiveMember.GetPayTypeAsString().GetString(), FALSE);	  //消费模式
	pRow->GetItem(10)->SetValue(ActiveMember.GetAdvancePaymentAsString().GetString(), FALSE);  //预付款
	pRow->GetItem(11)->SetValue(ActiveMember.GetCreditProcess().GetString(), FALSE);      //明细
	
	//姓名|证件号码
	pRow->GetItem(12)->SetValue(ActiveMember.GetUserName(TRUE).GetString(), FALSE);       //用户姓名
	pRow->GetItem(13)->SetValue(ActiveMember.GetPersonalID().GetString(), FALSE);         //证件号码
...
//先从视图中移除所有用户，锁定ActiveMemberList的CriticalSection，遍历ActiveMemberMap，再将所有ActiveMember加入到视图中
void CActiveMemberGrid::RedrawAllActiveMembers()
{
	...
	CSingleLock lock(&m_lpActiveMemberList->ActiveMemberListLock, TRUE); //为ActiveMemberList的CriticalSection对象上锁，只允许当前线程访问

	m_lpActiveMemberList->StartEnumActiveMember();   // 返回遍历的起点

	while (m_lpActiveMemberList->GetEachActiveMember(ActiveMember)) //遍历ActiveMemberMap，将所有的ActiveMember加入到视图中
	{
		AddNewRow(ActiveMember);
	}
}

//根据需要更新的索引列表，进行增加/更新/删除。不在需要更新的索引列表中的元素则不予理会
...
void CActiveMemberGrid::AddNewRow(CActiveMember &ActiveMember,  BOOL bAtFront)
{
	...
	{
		if(CActiveMember::EStatus_LocalOnline != ActiveMember.GetStatus() && CActiveMember::EStatus_CenterOnLine != ActiveMember.GetStatus())
		{
			return ;
		}
	}
