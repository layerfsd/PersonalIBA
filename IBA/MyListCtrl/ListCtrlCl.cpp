...
#undef SubclassWindow // windowx.h存在SubclassWindow的宏，将它取消才不会影响下面的代码：m_Header.SubclassWindow(pHeader->GetSafeHwnd());
...
void CListCtrlCl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	for ( int nCol=0; GetColumn(nCol, &lvc); nCol++)
	{
		...
		if ( lpDrawItemStruct->itemState & ODS_SELECTED )
		{
			...
			pDC->SetTextColor(RGB(255, 255, 255)) ;	// 选中的时候字体为白色
		}
		else
		{
			COLORREF color;
		...
		rcItem.left += 5;	// 加5个像素，不要太靠近边缘线qsc
...
bool CListCtrlCl::FindColFont(int col,LOGFONT& lf) //查找列字体
{
	int flag = 0;
...
void CListCtrlCl::SetColFont(int col,const LOGFONT* pLogFont ) //设置列字体
{
