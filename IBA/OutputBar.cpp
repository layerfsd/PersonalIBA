...
BOOL CIBAOutputBar::AddCheckOutLog(COperationInfo& OperationInfo)
{
	...
	case CIBAGlobal::emOpTypeBrokenReturnVip:
		{
			m_listCheckOut.InsertItem(0, COleDateTime::GetCurrentTime().Format(_T("%H:%M:%S")));
			CString strTmp;
			strTmp = CNetBarConfig::GetInstance()->GetUserClassName(OperationInfo.GetClassId());
...
void CIBAOutputBar::AddOperationLog(COperationInfo& OperationInfo)
{
	...
	case CIBAGlobal::emOpTypeRoomReturn://包房用户结账
		{
			m_wndListInfo.SetItemText(0, 1,CIBAGlobal::GetOperationName(OperationInfo.GetOperationType()));
	...
	case CIBAGlobal::emOpTypeBrokenReturn: // 本地退款
		{
			m_wndListInfo.SetItemText(0, 1, CIBAGlobal::GetOperationName(OperationInfo.GetOperationType()));
	...
	case CIBAGlobal::emOpTypeMakeupLocalCredit://上报本地充值
		{
			m_wndListInfo.SetItemText(0, 1, CIBAGlobal::GetOperationName(CIBAGlobal::emOpTypeMakeupLocalCredit));
			m_wndListInfo.SetItemText(0, 2, OperationInfo.GetUserName());
			m_wndListInfo.SetItemText(0, 3, OperationInfo.GetNetId());
	...
	case CIBAGlobal::emOpTypeMakeupLocalReturn://上报本地退款
		{
			m_wndListInfo.SetItemText(0, 1, CIBAGlobal::GetOperationName(CIBAGlobal::emOpTypeMakeupLocalReturn));
			m_wndListInfo.SetItemText(0, 2, OperationInfo.GetUserName());
			m_wndListInfo.SetItemText(0, 3, OperationInfo.GetNetId());
	...
	case CIBAGlobal::emOpTypeMakeupLocalReturnVip://上报本地结账
		{
			m_wndListInfo.SetItemText(0, 1, CIBAGlobal::GetOperationName(CIBAGlobal::emOpTypeMakeupLocalReturnVip));
			m_wndListInfo.SetItemText(0, 2, OperationInfo.GetUserName());
			m_wndListInfo.SetItemText(0, 3, OperationInfo.GetNetId());
...
void CIBAOutputBar::AddLocalActiveLog(CActiveMember* lpActiveMember)
{
	m_wndListActive.InsertItem(0, COleDateTime::GetCurrentTime().Format(_T("%H:%M:%S")));

	//m_wndListActive.SetItemText(0, 1, lpActiveMember->GetUserName(TRUE) + _T("(") + lpActiveMember->GetSexAsString()  + _T(")"));

	m_wndListActive.SetItemText(0, 1, lpActiveMember->GetUserName(TRUE));

	m_wndListActive.SetItemText(0, 2, lpActiveMember->GetNetId());

	CString strTmp;//用户级别
	strTmp = CNetBarConfig::GetInstance()->GetUserClassName(lpActiveMember->GetClassId());
	m_wndListActive.SetItemText(0, 3, strTmp);
	...
	m_wndListActive.SetItemText(0, 6, lpActiveMember->GetPersonalID());
...
void CIBAOutputBar::AddUpLoadingLocalRecordLog(CString strLog)
{
	m_wndListActive.SetItemText(0,6,LPCTSTR(strLog));
}
...
BOOL CIBAOutputBar::CreateOperationList()
{
	...
	const UINT Width[] = {70, 120, 100, 150, 60, 500};
...
/*************************************************
Function:       // OnOperationLog
Description:    // 响应WM_OPERATIONLOG，输出操作记录
Calls:          //
Table Accessed: //
Table Updated:  //
Input:          // wp - 0：CheckOut记录 1：Active记录 2：QueryConsume记录 3：上传本地操作记录消息
                // lp - 消息传递进来的COperationInfo&
Output:         //
Return:         //
Others:         //
*************************************************/
LRESULT CIBAOutputBar::OnOperationLog(WPARAM wp, LPARAM lp)
{
	...
	case 1:
		...
		{
			CActiveMember* lpActiveMember = (CActiveMember*)lp;
			AddLocalActiveLog(lpActiveMember);
		}
	...
	case 3:
		{
			COperationInfo* pOperationInfo = (COperationInfo*)lp;
			AddUpLoadingLocalRecordLog(pOperationInfo->GetDescription());
		}
	}
...
void CIBAOutputBar::OnExportExcel()
{
	...
	sTempPath.ReleaseBuffer(MAX_PATH);

	CString strFullPath;
	strFullPath.Format(_T("%s%s"), sTempPath, _T("结账记录表.xls"));
