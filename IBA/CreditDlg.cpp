...
, m_nBundTimeClass(0), m_bMem(false)
{
...
BEGIN_MESSAGE_MAP(CCreditDlg, CNDDialog)
	...
	ON_BN_CLICKED(IDC_WX_CREDIT, OnBnClickedWxCredit)
END_MESSAGE_MAP()
...
BOOL CCreditDlg::OnInitDialog()
{
	...
	if (m_bAutoQuery)  //在为窗口SetNetId时赋值为TRUE,表示要自动查询
	{
...
void CCreditDlg::OnBnClickedButtonRefundment()
{
	...
	if (m_bHasLocalQuery)
	{
		...
		//else
		//{
			IBA_LOG(_T("Temp.CCreditDlg::OnBnClickedButtonRefundment.结账!"));
			CString strErrMsg;
			if(CDeregisterHelpper::DeregisterUser(m_ActiveMember.GetMemberID(),strErrMsg))
			{
				IBA_LOG(_T("CCreditDlg::OnBnClickedButtonRefundment自动退款失败.MemberId=%d,错误:%s"),m_ActiveMember.GetMemberID(),strErrMsg);
			}
		//}
	}
	else
...
void CCreditDlg::InitControls()
{
	...
	//微信充值
	GetDlgItem(IDC_WX_CREDIT)->EnableWindow(FALSE);
	//充值打印
...
void CCreditDlg::DeregisterUser(BOOL bAutoRefund)
{
	...
	{	
		...
		if(CIBAGlobal::emRealnameZD == CNetBarConfig::GetInstance()->GetAuditType())
		{
			...
			coi.nCheckoutType = m_bMem ? CIBAGlobal::cot_MemLogout : CIBAGlobal::cot_TempLogout;
...
void CCreditDlg::DoBundTimeOnLine()
{
	...
	BundleTimeListDlg.SetUserClassId(m_nUserClassID); // 2016-3-17 - qsc
...
void CCreditDlg::ShowControls(CBalanceMethod & Balance)
{
	...
	GetDlgItem(IDC_WX_CREDIT)->EnableWindow(TRUE); //显示微信充值按钮
...
BOOL CCreditDlg::DoLocalModeQuery()
{
	...
	if (m_ActiveMember.GetIsLocalUser())
	{	
		m_strGiveBack.Format(_T("%.2f"),(m_ActiveMember.GetAdvancePayment()-m_ActiveMember.GetLocalExpense())/100.0);
	}
	else
	{
		m_strGiveBack.Format(_T("%.2f"),((double)m_ActiveMember.GetAvailavleBalance())/100.0);
	}

	m_strTmpUserPwd = m_ActiveMember.GetPassword();
	...
	if (m_bOnLine) //在线
	{
		m_strResult.Format(IDS_USERONLINE, m_ActiveMember.GetTerminalID());//终端号
	}
	else
	{
		m_strResult.Empty();
	...
	if (!m_ActiveMember.GetIsLocalUser() && m_ActiveMember.IsMember())
	{
...
{
	CString strErrMsg = CCashierHelper::DoCredit_InLocal(m_ActiveMember, nCreditMoney);
...
void CCreditDlg::OnBnClickedWxCredit()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(TRUE);
	...
	{
		ShowToolTip(IDS_NETWORKERROR,IDC_WX_CREDIT,TRUE);
		return;
	}
	...
	UINT nCreditMoney = CIBAHelpper::DecT2Int(m_strCreditMoney);

	//金额为空或者为0都不可以
	if(m_strCreditMoney.IsEmpty() || nCreditMoney <= 0)
	{
		return; 
	}
	...
	//生成请求参数并DES加密
	CString strData = CIBAHelpper::GenQrRequestParam(&strAccount,&strPwd,nmemberId1,0,nNeedPay,nCreditMoney);
	...
	if(FALSE == CIBAHelpper::GetQrData(&strData,&strQr,&nStatusCode))
	{
		...
		case 1:
			...
			return;
		case 2:
			ShowToolTip(IDS_WRONGRESPONSE,IDC_WX_CREDIT,TRUE);
			return;
		case 3:
			ShowToolTip(IDS_RESPONDFALSE,IDC_WX_CREDIT,TRUE);//中心返回失败
			return;
		case 4:
			ShowToolTip(IDS_RESPONDFALSE,IDC_WX_CREDIT,TRUE);//中心返回url为空
			return;
		default:
			return;
		}
	...
	qrData.strPrice.Format(_T("%.2f"),(float)nCreditMoney/100.0);
