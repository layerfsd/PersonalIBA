...
public:
	CGdiPlusBitmap()							{ m_pBitmap = NULL; m_bLoadSuccess = false; }
	CGdiPlusBitmap(LPCWSTR pFile)				{ m_pBitmap = NULL; m_bLoadSuccess = false; Load(pFile); }
	...
	bool Load(LPCWSTR pFile)
	{
		...
		m_bLoadSuccess = m_pBitmap->GetLastStatus() == Gdiplus::Ok;
		return m_bLoadSuccess;
	}

	bool IsLoadSuccess()	{ return m_bLoadSuccess; }
	operator Gdiplus::Bitmap*() const			{ return m_pBitmap; }
	UINT GetWidth() { return m_pBitmap->GetWidth();}
	UINT GetHeight() { return m_pBitmap->GetHeight();}
	// 2014-10-28 - qsc
	void SetLoadSuccess(bool newVal = true) { m_bLoadSuccess = newVal; }
private:
	bool m_bLoadSuccess;
};
