...
void CComputerInfo::CheckStatus()
{
	...
	if(m_ComputerStatus != nRet)		// 状态改变的时候要刷新场景视图
	{
		if(theApp.GetIBAView())
			::PostMessage(theApp.GetIBAView()->GetSafeHwnd(), WM_UPDATESEAT, 0, 0);
	}
...
void CComputerInfo::SetComputerStatus(ECOMPUTERSTATUS newVal)
{
	...
	if(m_ComputerStatus != newVal)		// 状态改变的时候要刷新场景视图
	{
		if(theApp.GetIBAView())
			::PostMessage(theApp.GetIBAView()->GetSafeHwnd(), WM_UPDATESEAT, 0, 0);
	}
...
Description:    //清除与用户相关所有信息(用户信息,费用信息,网吧信息,及部分终端信息)
Other:			//只剩下大部分终端信息和终端信息上报中心时间(time_t m_lCheckTime)
*************************************************/
...
//	m_LastNetBarID = 0;
//	m_LastAccountType = 0;
//	m_strComputerIP.Empty();
//	m_strMac;				// Mac地址 - Attach时设为空字符串，以后在上报线程中设置为被上报的终端IP
...
CComputerInfo::ECOMPUTERSTATUS CComputerInfo::GetComputerStatus() const
{
	if (m_ComputerStatus == ECOMPUTERSTATUS::ECS_THIEF) 
	{
		return ECOMPUTERSTATUS::ECS_ONLINE;
	}
	return m_ComputerStatus;
}
