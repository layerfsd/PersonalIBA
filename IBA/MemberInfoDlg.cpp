...
, m_nQueryStyle(0),m_nItemCount(0)
{
...
BOOL CDlgMemberInfo::OnInitDialog()
{
	...
	SetMemberInfoToList(m_nQueryStyle);  //如果m_nQueryStyle = 1则从本地数据库Member表中查询NetId对应的数据

	m_nItemCount = m_MemberInfoList.GetItemCount();
	...
	else if (m_MemberInfoList.GetItemCount() == 1)  //进入这一条
	{
...
// 从本地查找数据
void CDlgMemberInfo::FindInfoFromLocal(BOOL bValidUser, BOOL bFullMatch /*= FALSE*/)
{
	//账号非空
	if (GetNetId().IsEmpty())
	{
	...
	if(bFullMatch)
	{
		strSQL.AppendFormat(_T("WHERE serialNum = %s"), GetNetId());
	}
	else
	{
...
// 从已激活用户列表查找数据
void CDlgMemberInfo::FindInfoFromActiveMemberList()
{
	//账号非空
	if (GetNetId().IsEmpty())
	{
	...
	strSQL.Append(_T("SELECT * FROM ActiveMember "));

	strSQL.AppendFormat(_T("WHERE netId LIKE '%%%s'"), GetNetId());
	...
	strSQL.Append(_T(" and activationTime > '2011-1-1 0:0:0'"));	

	strSQL.Append(_T(" ORDER BY activationTime DESC"));

	CIBALog::GetInstance()->WriteFormat(CIBALog::ELT_SQL, _T("FindInfoFromActiveMemberList:%s"), strSQL);
