...
{
	m_lpCommandProcessor = new CCommandProcessor(this); //new CMD处理器并绑定到当前Server
	m_lpBufferProcessor = new CBufferProcess(BODY_BUFFER_LEN, m_lpCommandProcessor);//new Buffer处理器并绑定到CMD处理器
	m_lpSocket = new CClientSocket(m_lpBufferProcessor); //new ClientSocket并绑定Buffer处理器
...
{
	IBA_LOG(_T(" CLocalServer::LocalCheckOutUser - Start"));
	...
	IBA_LOG(_T(" CLocalServer::LocalCheckOutUser - End"));
}
