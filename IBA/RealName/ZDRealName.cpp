	...
	if (m_nMemberId > 0) //老卡，先激活
	{
		...
		if(IK_ID_OPENUSER == nType)
		{
			...
			{	
				...
				{
					CString strErrMsg;
					if (CDeregisterHelpper::DeregisterUser(CCurrentNetBarUser::GetInstance()->Balance.GetMemberId(),strErrMsg))//自动退款了
					{
					...
					{
						IBA_LOG(_T("CZDRealName::OnOpenCloseUser自动退款失败.MemberId:%d,错误:%s"),CCurrentNetBarUser::GetInstance()->Balance.GetMemberId(),strErrMsg);
					}
				}
			}
	...
	{	
		...
		{	//满足自动退款条件
			if (CDeregisterHelpper::AllowAutoRefund(CCurrentNetBarUser::GetInstance()->Balance))
			{
				...
				{
					IBA_LOG(_T("CZDRealName::OnCloseUser自动退款失败.MemberId:%d,错误:%s"),CCurrentNetBarUser::GetInstance()->Balance.GetMemberId(),strErrMsg);
				}
			}
...
BOOL CZDRealName::CheckIn(LPCTSTR szTermID, LPCTSTR szComputerIP, LPCTSTR szNetId, LPCTSTR szPwd, int nMemberId)
{
	...
	IBA_LOG(_T("Temp.CZDRealName::CheckIn AddLastUserInfo"));
