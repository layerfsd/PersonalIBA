...
BOOL SendMsgToSureName(UINT nMsgType)
{
	HWND hdSureName;
	hdSureName=::FindWindow(NULL, _T("任子行实名登记管理系统"));
	if (hdSureName==NULL)
	{
		AfxMessageBox(_T("找不到实名登记管理系统"));
		return FALSE;
	}
	BOOL bSend=::PostMessage(hdSureName,WM_USER_IDCARD_MANAGE,
		nMsgType,0);
	if (bSend==FALSE)
	{
		AfxMessageBox(_T("发送消息给实名登记管理系统失败！"));
		return FALSE;
	}

	//CString str;
	//str.Format(_T("发送消息给实名登记管理系统成功，消息类型：%d"), nMsgType);
	//AfxMessageBox(str);
	return TRUE;
}
