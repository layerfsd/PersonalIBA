...
INT CWDRealName::OnRealNameMsg(UINT nMsgType, LPVOID lpData /*= NULL*/, DWORD dwLen /*= 0*/)
{
	...
	{
		IBA_LOG0(_T("Abnormal.WD:OnRealNameMsg : s_NetBarUser.NetId is empty"));
		return 0;
	}
	...
	{
		OnLocalRealName(nMsgType,m_NetBarUser.GetNetId());
		return 0;
	}
	...
	{	
		...
		{	
			...
			{
				CString strErrMsg;
				if (CDeregisterHelpper::DeregisterUser(CCurrentNetBarUser::GetInstance()->Balance.GetMemberId(),strErrMsg))
				{
				...
				{
					IBA_LOG(_T("CWDRealName::OnRealNameMsg自动退款失败.MemberId:%d,错误:%s"),CCurrentNetBarUser::GetInstance()->Balance.GetMemberId(),strErrMsg);
				}
			}
		}
	...
	else
	{
		...
		IBA_LOG(_T("Temp.CWDRealName::OnRealNameMsg - AutoMatchIDCard Start"));
		if (AutoMatchIDCard(m_NetBarUser))
		{
			IBA_LOG(_T("Temp.CWDRealName::OnRealNameMsg - AutoMatchIDCard Result = 1"));
			AfxGetMainWnd()->BringWindowToTop();
			AfxGetMainWnd()->SetForegroundWindow();

			IBA_LOG(_T("Temp.CWDRealName::OnRealNameMsg - OnOldUser Start"));
			OnOldUser(m_NetBarUser);
			IBA_LOG(_T("Temp.CWDRealName::OnRealNameMsg - OnOldUser End"));
		}
		else
		{
			IBA_LOG(_T("Temp.CWDRealName::OnRealNameMsg - OnNewUser Start"));
			OnNewUser(m_NetBarUser);
			IBA_LOG(_T("Temp.CWDRealName::OnRealNameMsg - OnNewUser End"));
		}
	}
