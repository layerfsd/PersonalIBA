...
BOOL CIBARealName::CreateRealName()
{
	...
	// 2014-5-15 - qsc 若中心没有配置实名，那么就从配置文件读取实名类型
	if(0 == CNetBarConfig::GetInstance()->GetAuditType())
	{
		CNetBarConfig::GetInstance()->SetAuditType(
			CIBAConfig::GetInstance()->GetAuditType());
	}
...
//		break;
//
//	case CIBAGlobal::emRealnameJQ:
//		m_lpRealName = new CJQRealName;
//		break;
//
//	case CIBAGlobal::emRealnameZD:
//		m_lpRealName = new CZDRealName;
//		break;
//	}
//
//#else
...
{
	IBA_LOG(_T("实名下机.CIBARealName::AuditUserCheckOut"));
	CLocalServer::GetInstance()->LocalAuditUserQuit(strTermID, strIP, nMemberID);
}

void CIBARealName::CheckOut(const NS_REALNAME::CheckoutInfo* pCheckOutInfo )
{
	if (m_lpRealName != NULL)
	{
		m_lpRealName->OnCheckOut(pCheckOutInfo);
	}
}
