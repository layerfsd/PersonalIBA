...
CGLWRealName::CGLWRealName(void)
{
	...
	CIBAHelpper::WriteRegisterIsFN(TRUE);
}

CGLWRealName::~CGLWRealName(void)
{
	CIBAHelpper::WriteRegisterIsFN(FALSE);
}
	...
	{	
		...
		{	//满足自动退款条件
			if (CDeregisterHelpper::AllowAutoRefund(CCurrentNetBarUser::GetInstance()->Balance))
			{
				...
				{
					IBA_LOG(_T("CGLWRealName::OnCloseUser自动退款失败.MemberId:%d,错误信息:%s"),CCurrentNetBarUser::GetInstance()->Balance.GetMemberId(),strErrMsg);
				}
			}
...
BOOL CGLWRealName::ReadUserInfo(CNetBarUser & NetBarUser)
{
	...
	NetBarUser.SetActivationType(0);
