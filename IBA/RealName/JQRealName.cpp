...
INT CJQRealName::OnRealNameMsg(UINT nMsgType, LPVOID lpData /*= NULL*/, DWORD dwLen /*= 0*/)
{
	...
	if (0 == nMsgType)
	{
		IBA_LOG(_T("CJQRealName::OnRealNameMsg"));
...
{
	IBA_LOG(_T("CJQRealName::OnCloseUser"));
	...
	{	
		...
		{	//满足自动退款条件
			if (CDeregisterHelpper::AllowAutoRefund(CCurrentNetBarUser::GetInstance()->Balance))
			{
				...
				{
					IBA_LOG(_T("CJQRealName::OnCloseUser自动退款失败.MemberId:%d,错误:%s"),CCurrentNetBarUser::GetInstance()->Balance.GetMemberId(),strErrMsg);
				}
			}
