...
public:
	...
	enum EMemberStatus{
		...
		EStatus_LocalOnline =1,  //本地在线 
		EStatus_CenterOnLine =2, //中心在线 
		EStatus_AutoReturn =3,   //自动退款 
		EStatus_RoomAutoReturn = 4 //包房自动退款
	};
	// 用户的付款类型
	enum EMemberPayType{
...
public:
	...
	GProperty( UpdateDataTime, COleDateTime ); // 19700101/上次Iserver更新时间/包时到期时间/包房到期时间/上次下机时间
	...
	GProperty( IsLocalUser,            BOOL ); // 2011/07/08-8201-gxx: 是否为本地用户，指本地注册的
