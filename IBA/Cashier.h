...
public :
	...
	BOOL MonitorAuditProxy();

public :

	virtual BOOL CheckIn(CCashRegisterLoginMethod & CashRegisterLogin, CString strOperator);//登入
	virtual BOOL CheckIn_atLogin(CCashRegisterLoginMethod_atLogin & CashRegisterLogin, CString strOperator);//登陆时登入(即开启IBA时进行登入),区别只是失败时,会自动弹出提示和进行尝试
	...
	virtual BOOL MakeupCredit(CCreditMethod & Credit);   //2015-0915 liyajun 地模式下充值，网络恢复后的补充值，不需要再次更新ActiveMember对象及表的内容
	virtual BOOL DoMemberInfo(CMemberInfoMethod & MemberInfo);
	virtual BOOL DoDeregister(CDeregisterMethod & Deregister,UINT nReturnMode = 0);//0:正常退款 1:退款不销户 2:退包时后余额
	...
	virtual BOOL DoCheckout2(CCheckout2Method & Checkout2, BOOL bClearData=TRUE);
	...
	virtual BOOL DoBuckleAmount(CBuckleAmountMethod& BuckleAmountMethod);

	// 2011/07/13-8201-gxx: 
	BOOL LocalModeLogin();

	// 2014-8-11 - qsc
	void SetCenterAddr(CCashRegisterLoginMethod& CashRegisterLogin);
};
