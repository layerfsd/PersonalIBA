...
public :
	...
	*是否有网咖
	*/
	UINT GetIsInternetCafe();

			/**
	*网咖地址
	*/
	CString GetCafeAddress();
};

class CCashRegisterLoginMethod_atLogin : public CCashRegisterLoginMethod
{
public :
	CCashRegisterLoginMethod_atLogin(void);
	~CCashRegisterLoginMethod_atLogin(void);

public:
	BOOL ExecuteCheckin();						//执行请求，有对应特殊逻辑
};
