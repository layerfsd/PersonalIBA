...
// 2015/4/23 tmelody
// { 根据包时 id 获取起止时间
BOOL CBundleTimeListMethod::GetBundleIdTime(INT nId, CString& strStart, CString& strEnd)
{
	INT nCount = m_BundleTimeList.GetCount();
	if (nCount <= 0) return FALSE;

	for (INT i = 0; i < nCount; ++i)
	{
		if (m_BundleTimeList.GetAt(i).GetBundleId() == nId)
		{
			strStart = m_BundleTimeList.GetAt(i).GetStartTime();
			strEnd = m_BundleTimeList.GetAt(i).GetEndTime();
