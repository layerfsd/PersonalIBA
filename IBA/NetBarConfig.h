...
struct QrData 
{
	UINT nCode; //0 Logo 1 Clean 2 Success 3 具体数据
	CString strNumber;
	CString strPrice;
	CString strLink;
};
...
public :
	...
	std::map<CString, UINT> m_TermIDPCClassMap; //终端机器与区域ID的集合
...
public:
	...
	BOOL GetIsInternetCafe() const {return m_bIsInternetCafe;}//是否有网咖
	void SetIsInternetCafe(UINT newVal) {m_bIsInternetCafe = (BOOL)newVal;} 

	CString GetAddressInternetCafe() const {return m_strAddressInternetCafe;}//网咖地址
	void SetAddressInternetCafe(CString newVal) {m_strAddressInternetCafe = newVal;} 

private:

	UINT m_nStoreInAmount; //入库金额
	BOOL m_bIsInternetCafe; //是否有网咖
	CString m_strAddressInternetCafe; //网咖地址
...
public:
	NS_COMMON::CThreadTemplate<CNetBarConfig> *m_threadManipulateHaowangbao; // 操作好旺宝的线程
	void SetQrData(QrData *pData); //设置要显示的内容
	CRITICAL_SECTION m_csHaowangbao;
	UINT m_HWBRefreshInterval;

private:
	void ShowQrCode();//操作好旺宝
	QrData m_qrData;//要显示在好旺宝的内容
