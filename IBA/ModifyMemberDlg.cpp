...
void CModifyMemberDlg::OnOldCard(UINT nCardId, UINT nMemberId)
{
	...
	GetDlgItem(IDC_COMBO_IDTYPE)->EnableWindow(TRUE);
	GetDlgItem(IDC_BUTTON_MODIFY)->EnableWindow(TRUE);
...
void CModifyMemberDlg::SetUserInfo(CBalanceMethod & Balance)
{
	...
	// 2014-3-3 - qsc
	m_nSex = Balance.GetSex();
	m_nIdType = Balance.GetIdType();
	m_strIdCardSN = Balance.GetSerailNum();
	// 2014-2-27 - qsc 接口新增加的字段
	m_strTelNum = Balance.GetTel();
	m_strNation = Balance.GetJob();
	m_strAddress = Balance.GetAddress();
	m_strIdDepart = Balance.GetIdDepart();
...
void CModifyMemberDlg::UpdateUI()
{
	for (INT i = 0; i < m_cboIDType.GetCount(); i++)
	{
		if (m_nIdType == m_cboIDType.GetItemData(i))
		{
			m_cboIDType.SetCurSel(i);
			break;
		}
	}
	m_cboSex.SetCurSel(m_nSex);
	UpdateData(FALSE);
