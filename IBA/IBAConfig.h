...
private :
	...
	BOOL m_bNotShowChgDutyBtn;	// 不显示交班按钮
	...
	BOOL m_bCanNotTermConsumeQuery;	// 这个只在配置文件手动写入

	INT m_nAuditType;	// // 2014-5-15 读取实名配置 这个只在配置文件手动写入

	CString m_strFixReserve;
public :
	...
	BOOL GetNotShowChgDutyBtn() const { return m_bNotShowChgDutyBtn; }
	void SetNotShowChgDutyBtn(BOOL newVal) { m_bNotShowChgDutyBtn = newVal; }

	UINT GetShiftReportType() const { return m_nShiftReportType; }
	void SetShiftReportType(UINT newVal) { m_nShiftReportType = newVal; }
	
	BOOL GetCanNotTermConsumeQuery() const { return m_bCanNotTermConsumeQuery; }
	void SetCanNotTermConsumeQuery(BOOL newVal) { m_bCanNotTermConsumeQuery = newVal; }

	BOOL GetAuditType() const { return m_nAuditType; }
	void SetAuditType(BOOL newVal) { m_nAuditType = newVal; }
	
	CString GetFixReserve() const { return m_strFixReserve; }
	void SetFixReserve(CString newVal) { m_strFixReserve = newVal; }
private :
...
public:
	...
	// 2013-3-7 qsc
	GProperty(IsShowShiftCash, BOOL);						 // 是否显示"查询统计"菜单下的"收银员交款查询"
	
	// 2013/09/12-qsc:
	GProperty(CameraIndex, UINT);							// 聚宝科技开户拍照的时候摄像头的索引
	
	// 2014-10-10 - qsc 智多CRC
	GProperty(ZdCrc, DWORD);							// 聚宝科技开户拍照的时候摄像头的索引
private :
...
public:
	...
	//2015-0906 liyajun 获取是否加载过IBAConfig的标识
	BOOL GetHasLoaded() const {return m_bHasLoaded;}
};
