...
private:
	...
	void ActivateWndByComputerInfo(CComputerInfo& Info); //根据ComputerInfo来弹出对应的用户充值/退款窗口
	void PopupWindow_UnActiveMember(CString strNetId); //为未激活用户弹出对应窗口
	void PopupWindow(CActiveMember& ActiveMember);//弹出对应窗口

	//new
	BOOL QueryIDs_PartialMatch(CString strNetId,CString &strResultNetId); //从本地库(Member表)查找有效用户	
	BOOL QueryIDs_PartialMatch(CString strNetId,UINT &nMemberId); //从本地库(Member表)查找有效用户	
	BOOL Activation(CString strNetId, UINT &MemberID);//激活用户
	BOOL QueryActiveUser(CString strNetId,UINT &nMemberId); //从本地库ActiveMember表中查找匹配用户
public:
