...
void CJXCRetailOnlineDlg::OnBnClickedOk()
{
	...
	if (RetailOnline.GetStatusCode() == 0)
	{
		...
		if (BST_CHECKED == IsDlgButtonChecked(IDC_CHECK_ONLINE_ISPRINT))
		{
			TCHAR szBuf[MAX_PATH] = {0};
			GetDlgItemText(IDC_EDIT_IDCARDSNO, szBuf, MAX_PATH);
...
void CJXCRetailOnlineDlg::PrintTicket(CString strAccount)
{	
	CString strTicket, strPrint;
	CIBAPrint::GetShoppingTicket(strTicket, FALSE);
