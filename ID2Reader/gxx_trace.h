...
#define GXX_LOG_CREATE(logFilePath,nLimitDays) __gxx_log(logFilePath,nLimitDays)
#define GXX_LOG_CREATEEX(logFilePath,nLimitDays,bShowLogType,bDetailLog) __gxx_log(logFilePath,nLimitDays,bShowLogType,bDetailLog)
...
public:
    ...
    void operator ()(const wchar_t *tcszLogFilePath,int nLimitDays=-1, bool bShowLogType=false, bool bDetailLog=true);
...
private:
	...
	BOOL m_bInitPah;
	INT m_nLimitDay; // 只保留m_nLimitDay天的日志，-1表示保存所有的日志
	bool m_bShowLogType; // true: 输出日志的类型
	bool m_bDetailLog; // true: 每一条日志带毫秒和线程ID
};
