...
CReadHardwareInfo::CReadHardwareInfo(void)
{
}
...
CString CReadHardwareInfo::GetHostCode()
{
	_getCode();
	CString str = CMD5::MD5Buffer(_szCodeBuffer, _nBufferLen);
	str.MakeUpper();
	return str;
}

CString CReadHardwareInfo::GetHostCodeEx(CString& strHost)
{
	char szTR[32] = {'P',0, 'Z',0, 'X',0, 'N',0, 'M',0, 'L',0, 'U',0, 'H',0, 'S',0, 'I',0, '-',0, 'K',0, 'Y',0, 'D',0, ';',0, 'O',0};
	CString str = strHost;
	for (int i=0; i < strHost.GetLength(); i++)
	{
		char ch = strHost.GetAt(i);
		if ('0' <= ch && ch <= '9')
		{
			str.SetAt(i, szTR[(ch-'0')*2]);
		}
		else
		{
			str.SetAt(i, szTR[(10+ch-'A')*2]);
		}
	}
	return str;
}

BOOL CReadHardwareInfo::_getCode()
{
	BYTE* szSystemInfo = _szCodeBuffer;
	UINT& uSystemInfoLen = _nBufferLen;
	uSystemInfoLen = 0;
