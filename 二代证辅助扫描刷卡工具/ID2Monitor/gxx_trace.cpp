...
void __gxx_log__::operator ()(const wchar_t* tcszLogFilePath,int nLimitDays)
{
...
void __gxx_log__::DeleteHistory()
{
	if (m_nLimitDay < 1)
	{
		return;
	...
	COleDateTime dtDel = dt - COleDateTimeSpan(m_nLimitDay,0,0,0);
	
	WCHAR chBuf[512];
	swprintf(chBuf,L"%s%04d%02d%02d.log",m_chFilePath,dtDel.GetYear(),dtDel.GetMonth(),dtDel.GetDay());
...
void __gxx_log__::log_out( int iWay,const char* szOutFormat,... )
{
	...
	sprintf(outbuf,"%s %d-%d-%d %d:%02d:%02d %03d [%d]: %s\r\n",
		cOut,
...
void __gxx_log__::log_out( int iWay,const wchar_t* tcszOutFormat,... )
{
	...
	swprintf(outbuf,L"%s %d-%d-%d %d:%02d:%02d %03d [%d]: %s\r\n",
		tOut,
		stTime.wYear,stTime.wMonth,stTime.wDay,
		stTime.wHour,stTime.wMinute,stTime.wSecond,stTime.wMilliseconds,GetCurrentThreadId(),
		_wBuf);
...
void __gxx_log__::log_error( const wchar_t* tcszOutFormat,... )
{
	...
	va_list  args;
	va_start(args,tcszOutFormat);
	vswprintf(_wBuf,BUF_SIZE,tcszOutFormat,args);
	va_end(args);
