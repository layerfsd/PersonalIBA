...
public:
	...
	int GetLength(const_char_pointer pStr) const
	{
		int nSize = 0;
		for( ;*pStr; pStr++)
			nSize++;
	...
	int CompareNoCase(const_char_pointer lpcStr) const
	{
		int nLen1 = GetLength();
		int nLen2 = this->GetLength(lpcStr);
		if( nLen1 == 0 && nLen2 == 0)
			return 0;
		int nMin = nLen2 > nLen1?nLen1:nLen2;
		for(int i = 0 ; i < nMin ; i ++, lpcStr++)
		{
	...
	//{
	//	return CompareNoCase(str.GetData());
	//}
	...
	void ParseStringParams(
		std::map<CGxxString,CGxxString>& mapParams,
		CharType chAssign=(CharType)'=',
		CharType chSplit=(CharType)'&')
	{
		mapParams.clear();
		const_char_pointer lpParams = m_pData;
		while(*lpParams)
		{
			const_char_pointer lpStr = lpParams;
			int nCount = 0;
			...
			lpStr = lpParams;
			int nCount2 = 0;
			while (*lpStr != chAssign && nCount2 < nCount) {
				nCount2++;
				lpStr++;
			}
			CGxxString strKey;
			strKey.Append(lpParams, nCount2);
			// value可以为空
			CGxxString strValue;
			if (nCount > nCount2+1) {
				strValue.Append(lpStr+1,nCount - nCount2 - 1);
			}
	...
	unsigned long Parse16ToUINT32()
	{
		CGxxString str = Left(8);
		str.FillCharFromLeft(8,CharType('0'));
		str.ToUpper();
		unsigned long nTs[8] = {16*16777216,16777216,16*65536,65536,16*256,256,16,1};
		...
		for (int i=0; i < 8; i++)
		{
			...
			if (CharType('0')<=chH&&chH<=CharType('9'))
				...
				chVal = 10+(chH-CharType('A'));
			nRes += nTs[i] * (unsigned long)chVal;
		}
		return nRes;
	}
	unsigned __int64 Parse16ToUINT64()
	{
		CGxxString str = Left(16);
		str.FillCharFromLeft(16,CharType('0'));
		unsigned long nHight = str.Left(8).Parse16ToUINT32();
		unsigned long nLow = str.Right(8).Parse16ToUINT32();
		unsigned __int64 nVal = unsigned __int64(nHight)*65536*65536;
		nVal += nLow;
		return nVal;
	}
